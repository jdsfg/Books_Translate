...
到本书结束时，你将拥有一个评估行为经济学主张的实用框架：样本量（sample size）是多少？是否进行了预注册（pre-registered）？实验室效应（lab effect）是否在实地场景（field settings）中出现？隐含的机制（mechanism）是什么，它能否正确预测其他领域的现象？这个框架可能比任何单一发现都更有价值。

**目标读者**：经济学与商科学生；政策分析师；MBA 学生；对经济行为感兴趣的心理学家；希望了解流行行为经济学书籍背后学术深度的读者

**先修要求**：入门经济学（基本的供给/需求、效用、期望价值）有帮助，但不是必需的——关键概念会在正文中逐步引入

**学习时长**：90–120 小时（含练习与反思）